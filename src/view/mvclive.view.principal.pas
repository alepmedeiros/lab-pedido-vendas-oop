...
type
  ...
  private
    ...
    procedure LimparEditsPosAddProduto;

    procedure ValidaUsuario(TipoUsuario: Integer);
  public
    { Public declarations }
  end;
...
begin
  ...
  try
    FController
      .Produto
        .SetCodigo(StrToInt(edtCodigoProduto.Text))
      .Build.ListarPorId.DataSource(lDataSource);
    ...
    ClientDataSet1Unitario.AsCurrency := lDataSource.DataSet.Fields[2]
      .AsCurrency;
    ClientDataSet1quantidade.AsInteger := StrToInt(edtQuantidade.Text);
    ClientDataSet1.Post;

    LimparEditsPosAddProduto;
  finally
    lDataSource.Free;
  end;
end;
...
begin
  var
  lTotal := StrToCurrDef(String(Label1.Caption).Replace('R$', ''), 0) +
...
begin
  ClientDataSet1Total.AsCurrency :=
    (ClientDataSet1Unitario.AsCurrency * ClientDataSet1quantidade.AsInteger);
end;
...
begin
  ...
  try
    if not (edtCodigoCliente.Text = '') then
      FController.Cliente.SetCodigo(StrToInt(edtCodigoCliente.Text))
        .Build.ListarPorId.DataSource(lDataSource);

    if not lDataSource.DataSet.IsEmpty then
    begin
      edtNomeCliente.Text := lDataSource.DataSet.Fields[1].AsString;
      edtCodigoProduto.SetFocus;
      exit;
    end;

    raise Exception.Create('Não foi possivel encontrar o cliente');
  finally
    lDataSource.Free;
  end;
end;

procedure TFormPrincipal.edtCodigoClienteKeyPress(Sender: TObject;
  var Key: Char);
begin
  SomenteNumeros(Key);
end;
...
begin
  ...
  try
    if not (edtCodigoProduto.Text = '') then
      FController.Produto.SetCodigo(StrToInt(edtCodigoProduto.Text))
        .Build.ListarPorId.DataSource(lDataSource);
    ...
    raise Exception.Create('Não foi encontrado nenhum produto');
  finally
    lDataSource.Free;
  end;
end;

procedure TFormPrincipal.edtCodigoProdutoKeyPress(Sender: TObject;
  var Key: Char);
begin
  SomenteNumeros(Key);
end;
...
begin
  var
  lLogin := TFormLogin.Create(nil);
  try
    lLogin.ShowModal;

    ValidaUsuario(lLogin.TipoUsuario);
  finally
    lLogin.Free;
  end;
...
procedure TFormPrincipal.LimparEditsPosAddProduto;
begin
